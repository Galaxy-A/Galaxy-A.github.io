...
This repository is a static GitHub Pages knowledge base. The content index lives in `data/content.json`; detailed article bodies should live in Markdown files under `content/` or `assets/files/`. Small linked files live under `assets/files/`.
...
- `source`: relative path to a Markdown body. Use it for implementation details, commands, long notes, and shareable guides instead of embedding long structured content in JSON.
...
- Put long implementation content in Markdown and reference it with `source`; do not store long steps, command lists, or notes directly in JSON.
...
```powershell
wsl.exe bash -lc 'cd /mnt/<drive>/<path-to-project> && python3 -m json.tool data/content.json >/dev/null'
```