...
如果要装到 Cursor 全局 MCP，优先用 `ssh-mcp`。它的 Cursor/stdio 说明更明确，适合直接放进全局 `mcp.json`。

Cursor 并不是提前知道 `ssh-mcp` 安装在哪里，而是把配置里的 `command` 和 `args` 当作启动命令执行：

```text
npx -y ssh-mcp -- --host=... --port=22 ...
```
...
Cursor
→ 调用当前系统 PATH 里的 npx
→ npx 解析 npm 包名 ssh-mcp
→ 如果本地/缓存已有可用版本就复用
→ 如果没有就从 npm registry 临时下载
→ 读取包内 bin 映射 ssh-mcp -> build/index.js
→ 启动这个 CLI，作为 stdio MCP server 接入 Cursor
```

所以这份配置依赖的是 `npx` 能被 Cursor 启动，而不是 Cursor 自己知道 npm 包安装路径。若要避免临时下载，可以全局安装后改用固定入口：
...
但全局安装模式要求 `ssh-mcp` 这个命令所在目录已经进入 Cursor 可见的 `PATH`。稳定性上，`npx -y ssh-mcp` 更省路径维护；可控性上，固定全局入口更容易锁定运行来源。
